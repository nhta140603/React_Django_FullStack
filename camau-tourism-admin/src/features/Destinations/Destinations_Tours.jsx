...
export default function TourSchedulePage() {
  ...
  const [editDestSuggestion, setEditDestSuggestion] = useState([]);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      setStep(1);
      setTourForm({
        name: "", price: "", duration: "", max_people: "", min_people: ""
      });
      setSchedules([]);
      setTour({});
    } catch (error) {
  ...
  ///Suggestion input cho destination
  /// Nhập text => xử lý chuỗi nhập vào => gợi ý ra từ trường nhập vào nếu có không thì cho mảng rỗng
  /// Quản lý lưu state việc nhập input và cho ra suggestion
  const [query, setQuery] = useState("")
  const [destSuggestion, setDestSuggestion] = useState([])
  //// hàm để khi điền vào thì hiện ra suggestion theo input
  const getSuggestions = value => {
    const inputValue = value.trim().toLowerCase();
    return destination.filter(d => d.label.toLowerCase().includes(inputValue))
  }
  //// hàm để lấy giá trị
  const getValueSuggestions = suggestion => suggestion.label

  /// hàm render ra mỗi giá trị của một suggestion
  const renderSuggestion = suggestion => (
    <div className="flex items-center gap-2 py-1 px-2 w-200">
      <img
        src={suggestion.image}
        alt=""
        className="w-7 h-7 rounded-full object-cover border border-cyan-100 shadow-sm"
        style={{ minWidth: 28 }}
      />
      <span className="truncate font-medium text-cyan-900">{suggestion.label}</span>
    </div>
  );
  ...
  /// Mỗi khi hàm được gọi thì nó sẽ gọi tới
  const onSuggestionsFetchRequested = ({ value }) => {
    setDestSuggestion(getSuggestions(value))
  };
  ...
  const inputProps = {
    ...
    className: "border rounded-lg px-4 py-2 focus:ring-2 focus:ring-cyan-400"
  };
  const getEditSuggestions = value => {
    const inputValue = value.trim().toLowerCase();
    return destination.filter(d => d.label.toLowerCase().includes(inputValue));
  };
  const getEditSuggestionValue = suggestion => suggestion.label;
  const renderEditSuggestion = renderSuggestion; // Dùng lại hàm render cũ

  const onEditSuggestionSelected = (event, { suggestion }) => {
    setEditDest(i => ({ ...i, destination: String(suggestion.value) }));
    setEditQuery(suggestion.label);
  };

  const onEditSuggestionsFetchRequested = ({ value }) => {
    setEditDestSuggestion(getEditSuggestions(value));
  };

  const onEditSuggestionsClearRequested = () => {
    setEditDestSuggestion([]);
  };
  ...
  const editInputProps = {
    ...
    value: editQuery,
    onChange: (event, { newValue }) => {
      setEditQuery(newValue);
      setEditDest(i => ({ ...i, destination: "" }));
    },
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              ...
              <CurrencyInput
                ...
                onValueChange={(value) =>
                  setTourForm({ ...tourForm, price: value ? parseInt(value.replace(/,/g, ""), 10) : 0 })
                }
                    ...
                    <div className="flex-1 bg-cyan-50/60 backdrop-blur-xl px-6 py-5 rounded-2xl shadow-md border border-cyan-200 group-hover:border-cyan-400 transition-all flex items-center gap-6 hover:scale-[1.015]">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setEditQuery(item.destination.name);
                          setShowAdd(false);
                        }}
              ...
              <div className="flex flex-col md:flex-row items-end gap-5 bg-cyan-100/80 border border-cyan-300 rounded-2xl p-6 mt-3 shadow-lg">
                <div>
                  ...
                  <Autosuggest
                    ...
                    getSuggestionValue={getValueSuggestions}
                    ...
                    theme={{
                      ...
                      suggestionsContainer:
                        "absolute z-20 left-0 right-0 bg-white border border-cyan-100/80 rounded-xl mt-2 shadow-lg max-h-64 w-90 overflow-hidden",
                      suggestion:
                        "flex items-center gap-2 py-2 px-3 cursor-pointer transition-colors duration-100",
                      suggestionHighlighted: "bg-cyan-50"
                    }}
                  />
                </div>
                <div>
            ...
            {editIdx !== null && (
              <div className="flex flex-col md:flex-row items-end gap-5 bg-yellow-50 border border-yellow-300 rounded-2xl p-6 mt-3 shadow-lg">
                <div>
                  <label className="block text-sm font-semibold mb-1">Địa điểm</label>
                  <Autosuggest
                    suggestions={editDestSuggestion}
                    onSuggestionsFetchRequested={onEditSuggestionsFetchRequested}
                    onSuggestionsClearRequested={onEditSuggestionsClearRequested}
                    getSuggestionValue={getEditSuggestionValue}
                    renderSuggestion={renderEditSuggestion}
                    inputProps={editInputProps}
                    onSuggestionSelected={onEditSuggestionSelected}
                    highlightFirstSuggestion={true}
                    theme={{
                      container: "relative",
                      suggestionsContainer:
                        "absolute z-20 left-0 right-0 bg-white border border-yellow-100/80 rounded-xl mt-2 shadow-lg max-h-64 w-90 overflow-hidden",
                      suggestion:
                        "flex items-center gap-2 py-2 px-3 cursor-pointer transition-colors duration-100",
                      suggestionHighlighted: "bg-yellow-50"
                    }}
                  />
                </div>
                <div>
