...
export default function ProfileSidebar({ avatar, name }) {
  ...
  return (
    <>
      ...
      <aside 
        className={`
          fixed lg:sticky h-screen z-40
          transition-all duration-300 ease-in-out
          bg-gradient-to-tr from-blue-600 via-blue-500 to-teal-400 text-white
      ...
      >
        <button 
          className="hidden lg:block absolute -right-3 top-16 bg-white text-blue-600 rounded-full p-1 shadow-md hover:bg-gray-100 z-10"
        ...
        <nav className="flex-1 flex flex-col gap-1 px-2">
          {menuItems.map(item => (
            <Link
              key={item.key}
              to={item.path}
              className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-start'} gap-3 ${isCollapsed ? 'px-2' : 'px-4'} py-3 text-left font-medium hover:bg-white/20 rounded-xl transition-all duration-200 relative group ${
        ...
        {menuItems.slice(0, 5).map(item => (
          <Link
            ...
            className={`flex flex-col items-center justify-center py-1 px-2 ${
              selected === item.key ? 'text-blue-600' : 'text-gray-600'
            }`}
          >
