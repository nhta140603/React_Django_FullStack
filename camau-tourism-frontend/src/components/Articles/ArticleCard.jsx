...
export default function ArticleCard({ article, viewType = "grid" }) {
  if (viewType === "grid") {
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden h-full flex flex-col">
        ...
        <div className="p-2 sm:p-4 flex-1 flex flex-col">
          <div className="flex items-center gap-1 sm:gap-2 mb-1 sm:mb-2">
            <span className={`px-1.5 py-0.5 sm:px-2 sm:py-1 rounded text-[11px] sm:text-xs font-medium ${article.type === 'news' ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}`}>
          ...
          <Link
            to={article.type === "news" ? `/tin-tuc/${article.slug}` : `/su-kien/${article.slug}`}
            className="text-base sm:text-lg font-semibold hover:text-blue-600 mb-1 sm:mb-2"
          >
            {article.title}
          </Link>
          ...
          {article.type === 'event' && article.event_date && (
            <div className="mt-auto bg-blue-50 p-1.5 sm:p-2 rounded flex items-center gap-1 sm:gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    return (
      <div className="bg-white rounded-lg shadow overflow-hidden flex flex-row">
        ...
        <div className="p-2 sm:p-4 flex-1 flex flex-col">
          ...
          <Link
            to={article.type === "news" ? `/tin-tuc/${article.slug}` : `/su-kien/${article.slug}`}
            className="text-base sm:text-lg font-semibold hover:text-blue-600 mb-1 sm:mb-2 block"
          >
            {article.title}
          </Link>
          <div
