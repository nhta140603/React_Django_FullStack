...
function CountdownTimer({ eventDate }) {
  ...
  return (
    <div className="flex flex-col items-center mt-2 sm:mt-4">
      <h3 className="text-base sm:text-lg text-gray-700 mb-2">Sự kiện bắt đầu sau:</h3>
      <div className="grid grid-cols-4 gap-1 sm:gap-2 text-center">
        {["Ngày", "Giờ", "Phút", "Giây"].map((label, idx) => (
          <div
            key={label}
            className="bg-gradient-to-br from-cyan-600 to-cyan-700 p-2 sm:p-3 rounded-lg shadow"
          >
  ...
  if (isError || !event)
    return (
      <div className="text-center py-20">
        ...
        <h2 className="text-lg sm:text-xl font-semibold text-red-500">Không tìm thấy sự kiện</h2>
        <p className="text-gray-500 mt-1 sm:mt-2">Sự kiện có thể đã bị xóa hoặc không tồn tại</p>
      </div>
    );
        ...
        <div className="hidden md:block mt-4 sm:mt-6rounded-2xl p-2 sm:p-4 md:p-8">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2 sm:gap-5 border-b pb-4 sm:pb-8">
            <div className="flex-1">
              <div className="flex items-center gap-1 sm:gap-2 mb-2 sm:mb-3">
                {event.is_featured && (
                  <span className="inline-block bg-cyan-600 text-white text-xs px-2 py-0.5 rounded-full shadow">
                    Sự kiện nổi bật
                  </span>
                )}
                <span className="inline-block bg-yellow-100 text-yellow-800 text-xs px-2 py-0.5 rounded-full shadow-sm">
                  {formattedDate}
                </span>
              </div>
              <CountdownTimer eventDate={formattedDate} />
            </div>
            <div className="flex gap-1 sm:gap-2 mt-3 md:mt-0">
              <button
                onClick={handleAddToCalendar}
                className="flex items-center gap-2 px-2 py-1 sm:px-4 sm:py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow text-xs sm:text-base transition-colors"
              >
                <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                  <line x1="16" y1="2" x2="16" y2="6" />
              ...
              <button
                onClick={handleShareEvent}
                className="flex items-center gap-2 px-2 py-1 sm:px-4 sm:py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg shadow text-xs sm:text-base transition-colors"
              >
                <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                  <circle cx="18" cy="5" r="3" />
                  <circle cx="6" cy="12" r="3" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-6 mt-4 sm:mt-8">
            <div className="md:col-span-2">
              <div className="mb-5 sm:mb-8">
                <h2 className="text-base sm:text-xl font-bold text-cyan-900 mb-2 sm:mb-4 flex items-center">
                  <svg className="mr-1 sm:mr-2 w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    <path d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  Thông tin sự kiện
                </h2>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-4 mb-3 sm:mb-6">
                  <EventInfoCard
                    icon={
                      <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                ...
                <div>
                  ...
                  <div className="bg-white rounded-xl overflow-hidden transition-all">
                    <div className="p-1">
                      <div
                        className="prose max-w-none prose-img:rounded-xl prose-img:shadow-md prose-headings:text-cyan-700 prose-a:text-cyan-600 prose-a:underline prose-table:rounded-lg prose-table:shadow text-sm sm:text-base"
                        dangerouslySetInnerHTML={{
                          __html: event.description || "<p>Chưa có thông tin chi tiết.</p>",
                        }}
                      />
            ...
            <div className="md:col-span-1">
              <div className="md:sticky md:top-8 space-y-4 sm:space-y-6">
                ...
                {mapSrc && (
                  <div>
                    <h2 className="text-base sm:text-lg font-bold text-cyan-900 mb-2 sm:mb-3 flex items-center">
                      <svg className="mr-1 sm:mr-2 w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <path d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
                      </svg>
                      Vị trí
                    ...
                    <div className="rounded-xl overflow-hidden shadow">
                      <div className="w-full h-[120px] sm:h-[250px]">
                        <iframe
                          src={mapSrc}
                          className="w-full h-full min-h-[120px] sm:min-h-[250px] border-0"
                          frameBorder="0"
                          allowFullScreen
                          aria-hidden="false"
                          tabIndex="0"
                ...
                <div className="bg-cyan-50 p-3 sm:p-4 rounded-xl shadow-sm">
                  <h3 className="font-semibold text-cyan-800 mb-2 text-sm sm:text-base">
                    Lưu ý quan trọng:
                  </h3>
                  <ul className="text-xs sm:text-sm text-cyan-700 space-y-2">
                    <li className="flex items-start">
                      <svg className="w-4 h-4 sm:w-5 sm:h-5 mr-1 text-cyan-600 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
